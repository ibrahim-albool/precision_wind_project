    ...
    with torch.no_grad():
        ...
        while total_t < min_timesteps:
            ...
            while not done:  # and timesteps < max_traj_len:
                action = policy.forward(state, update_norm=True).numpy() + np.random.normal(0, noise,
                                                                                            size=policy.action_dim)
                state, _, done, _ = env.step(action)
                timesteps += 1
                total_t += 1


def eval_policy(model, env=None, episodes=1000, max_traj_len=400, verbose=True, visualize=False):
    if env is None:
...
def env_factory(dynamics_randomization, verbose=False, **kwargs):
    ...
    if verbose:
        print("Created environment env with arguments:")
        print("\tdynamics randomization: {}".format(dynamics_randomization))
    return partial(Environment, dynamics_randomization=dynamics_randomization)
