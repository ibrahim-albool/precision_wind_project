...
def run_experiment(args):
    ...
    while timesteps < args.timesteps:
      ...
      eval_reward = eval_policy(algo.actor, env, episodes=20, max_traj_len=args.traj_len, verbose=False, visualize=False)
      ...
      if logger is not None:
          save_crnt_actor = os.path.join(logger.dir, 'actor_current.pt')
          torch.save(algo.actor, save_crnt_actor)

      if logger is not None:
        logger.add_scalar('environment/kl', kl, itr)
        logger.add_scalar('environment/return', eval_reward, itr)
        logger.add_scalar('environment/actor_loss', a_loss, itr)
        logger.add_scalar('environment/critic_loss', c_loss, itr)
      itr += 1
